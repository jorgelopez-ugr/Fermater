...
function myOwnParser(html) {
    
    const fechasPrecios = new FechasPrecios();
    
    // Extraer todas las filas <tr>...</tr> completas
    const regexFilas = /<tr[^>]*>[\s\S]*?<\/tr>/gi;
    const filasCompletas = html.match(regexFilas) || [];
    
    let fechaActual = null;
    
    for (let i = 0; i < filasCompletas.length; i++) {
        const fila = filasCompletas[i];        
        // Verificar si es fila de fecha (tiene class="table-secondary")
        if (esFilaFecha(fila)) {
            // Fila de fecha
            const fecha = extraerFechaDeFila(fila);
            if (fecha) {
                fechaActual = fecha;
                fechasPrecios.listaPrecios.set(fechaActual, []);
            }
        } else {
            // Fila de datos
            if (fechaActual) {
                // Formato ejemplo de lo que seria una fila:
                // <tr><td><strong>AOVE - Noviembre</strong></td><td align="center">Picual</td><td align="right"><strong>4.450 €</strong></td></tr>
                const columnas = obtenerColumnasDeFila(fila);
                
                if (columnas.length >= 3) {                    
                    const variedad = extraerVariedadDeColumna(columnas[0]);
                    const precioTexto = extraerPrecioDeColumna(columnas[2]);
                    const valor = parsearPrecio(precioTexto);                    
                    const variedadPrecio = new VariedadPrecio(variedad, valor);
                    fechasPrecios.listaPrecios.get(fechaActual).push(variedadPrecio);
                }
            }
        }
    }

    return fechasPrecios;
}
